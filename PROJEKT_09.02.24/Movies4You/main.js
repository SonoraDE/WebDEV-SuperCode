...
const showAll = () => {
    ...
    filteredMovies.forEach((movie) => {
        const genre = movie[4].join(', ');
        let newMovie = `<div class="movie-card">
        <h3 class="title">${movie[0]}</h3>
        <p class="release">${movie[1]}</p>
        <p class="producer">${movie[2]}</p>
        <p class="movie-length">${movie[3]}</p>
        <p class="genre">${genre}</p>
        <p class="rating">${movie[5]}</p>
        </div>`
        movieSelect.innerHTML += newMovie;  
    });
}
...
const searchBar = () => {
    inputText = document.querySelector("#search-bar").value;
    console.log(inputText);
    filteredMovies = movies.filter((movie) => movie[0].toLowerCase().includes(inputText.toLowerCase()) || 
    movie[1] === inputText);
    errorFunction();
    showAll();
}
